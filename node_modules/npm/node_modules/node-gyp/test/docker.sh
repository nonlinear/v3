...
setup_container() {
  ...
  docker inspect "map-get($size, container)_id" &> /dev/null
  if [[ $? -eq 0 ]]; then
    echo "Found existing container [map-get($size, container)_id]"
  else
    # No such image, so make it
    echo "Did not find container [map-get($size, container)_id], creating..."
    docker run -i $base_container /bin/bash -c "$run_cmd"
    sleep 2
    docker commit $(docker ps -l -q) map-get($size, container)_id
  fi
}
